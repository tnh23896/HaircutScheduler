        ...
        $(document).ready(function() {
            ...
            function renderTimes(times) {
                ...
                if (times.length === 0) {
                    ...
                    // Nếu có thời gian có trạng thái "available", thêm thời gian vào #timeSelect
                    times.forEach(function(time) {
                        if (time.pivot && time.pivot.status === 'available') {
                            const radioOption = `
                            <label class="check mx-1">
                                <input type="radio" id="time_${time.id}" name="time_id" value="${time.id}">
                                <span>${time.time}</span>
                            </label>
                        `;
