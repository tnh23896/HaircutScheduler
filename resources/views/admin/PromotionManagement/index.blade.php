...
@section('content')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap xl:flex-nowrap items-center mt-2">
            @if(auth('admin')->user()->can('admin.PromotionManagement.create'))
                <a href="{{ route('admin.PromotionManagement.create') }}" class="btn btn-primary">Thêm mới mã giảm
                    giá</a>
            @endif
        ...
        <div class="intro-y col-span-12 overflow-auto 2xl:overflow-visible">
            <table class="table table-report -mt-2">
                ...
                <tr>
                    ...
                    @if(auth('admin')->user()->can('admin.PromotionManagement.edit') || auth('admin')->user()->can('admin.PromotionManagement.delete'))
                        ...
                        @if(auth('admin')->user()->can('admin.PromotionManagement.edit') || auth('admin')->user()->can('admin.PromotionManagement.delete'))
                            <td class="table-report__action w-56">
                                <div class="flex justify-center items-center">
                                    @if(auth('admin')->user()->can('admin.PromotionManagement.edit'))
                                        <a class="flex items-center mr-3"
                                           href="{{ route('admin.PromotionManagement.edit', $item->id) }}">
                                            <i data-lucide="check-square" class="w-4 h-4 mr-1"></i>
                                            Sửa </a>
                                    @endif
                                    @if(auth('admin')->user()->can('admin.PromotionManagement.delete'))
                                        <form class="delete-form"
                                              action="{{ route('admin.PromotionManagement.delete', $item->id) }}"
                                              method="POST">
                                            @csrf
                                            @method('DELETE')
